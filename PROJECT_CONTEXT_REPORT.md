...
Prepared in the repository, not yet scheduled: `scripts/run_daily_cycle.py` (run-once orchestrator, continues after a failed stage, exits non-zero if any failed; the four subprocess-based jobs now raise when their pipeline exits non-zero), `deploy/run_daily_cycle.ps1` (dated log under `logs\`), `deploy/register_daily_cycle_task.ps1` (registers `Wansoft_Pipeline_Diario` at 01:00, **disabled unless `-Enable`**), and `scripts/check_env.py` (read-only `.env` preflight that never prints secrets).
