...
def schedule_weekly_at(job, weekday, hour, minute=0):
    """
    Corre `job` una vez a la semana, el día `weekday` (0=lunes ... 6=domingo,
    igual que datetime.weekday()) a la hora local `hour:minute` (24h), sin
    importar a qué hora arrancó el scheduler.
    """
...
def start():
    ...
    # backlog de mapeo de productos Odoo -> Wansoft (inventory_mapping_dictionary),
    # domingo 11am -- semanal, no diario, porque el catálogo de productos
    # cambia mucho más lento que las compras/ventas del día a día. Alimenta
    # directamente el backlog de mapeo de Compras/Inventario (ver
    # docs/purchases-product-mapping-policy.md); nunca escribe 'approved' por
    # sí solo salvo un match de código literal, y nunca toca una fila que un
    # humano ya revisó (approved/rejected)
    schedule_weekly_at(run_product_mapping_backlog_job, weekday=6, hour=11, minute=0)
