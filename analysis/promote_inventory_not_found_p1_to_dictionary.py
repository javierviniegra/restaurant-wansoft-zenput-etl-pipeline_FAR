...
def promote_inventory_not_found_p1_to_dictionary():
    """
    Promueve P1 bridge al diccionario de inventory.

    Reglas:
    - approved_candidate -> approved
    - review_candidate -> pending_review
    - historical_candidate / historical_stock_review -> historical_only

    Importante:
    - NO trunca inventory_mapping_dictionary
    - Hace UPSERT con ON DUPLICATE KEY UPDATE
    """
    ...
    SELECT
        p1.odoo_product_id,
        p1.odoo_product_name,
        p1.category_name,
        p1.wansoft_code,
        p1.wansoft_product_name,
        p1.wansoft_department,
        p1.lifecycle_candidate,
        p1.similarity_score,
        p1.suggested_action
    FROM inventory_not_found_p1_bridge p1
    """

    df = pd.read_sql(query, conn)

    if df.empty:
        print("No hay registros P1 para promover.")
        conn.close()
        return

    df["mapping_status"] = df["suggested_action"].apply(map_status)
    df["mapping_source"] = "p1_bridge"
    ...
    for _, row in df.iterrows():
        rows.append((
            ...
            f"suggested_action={row.get('suggested_action')}; promoted_from=inventory_not_found_p1_bridge",
    ...
    print(f"Promovidos {len(rows)} registros de P1 a inventory_mapping_dictionary.")
