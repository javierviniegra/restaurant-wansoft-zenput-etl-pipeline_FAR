...
def promote_inventory_not_found_p2_to_dictionary():
    """
    Promueve P2 bridge al diccionario de inventory.
    ...
    SELECT
        p2.odoo_product_id,
        p2.odoo_product_name,
        p2.category_name,
        p2.wansoft_code,
        p2.wansoft_product_name,
        p2.wansoft_department,
        p2.lifecycle_candidate,
        p2.similarity_score,
        p2.suggested_action
    FROM inventory_not_found_p2_bridge p2
    """

    df = pd.read_sql(query, conn)

    if df.empty:
        print("No hay registros P2 para promover.")
        conn.close()
        return

    df["mapping_status"] = df["suggested_action"].apply(map_status)
    df["mapping_source"] = "p2_bridge"
    ...
    for _, row in df.iterrows():
        rows.append((
            ...
            f"suggested_action={row.get('suggested_action')}; promoted_from=inventory_not_found_p2_bridge",
    ...
    print(f"Promovidos {len(rows)} registros de P2 a inventory_mapping_dictionary.")
