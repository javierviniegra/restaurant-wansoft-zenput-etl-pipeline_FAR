...
def save_inventory_not_found_p2_bridge():
    df = build_inventory_not_found_p2_bridge()

    if df is None or df.empty:
        print("No hay bridge de P2 para guardar.")
        return

    conn = get_db_connection(target="wansoft")
    cursor = conn.cursor()

    cursor.execute("TRUNCATE TABLE inventory_not_found_p2_bridge")

    insert_sql = """
    INSERT INTO inventory_not_found_p2_bridge (
        odoo_product_id,
        odoo_product_name,
        category_name,
        wansoft_code,
    ...
    print(f"Insertados {len(rows)} registros en inventory_not_found_p2_bridge.")
